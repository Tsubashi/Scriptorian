...
public extension Catalog {
    ...
    class SubitemMetadataTable {
        ...
        static func from(row: Row) -> SubitemMetadata {
            return SubitemMetadata(itemID: row[itemID], subitemID: row[subitemID], docID: row[docID], docVersion: row[docVersion])
        }
    }
    ...
    public func subitemMetadataForItem(withID itemID: Int64) -> [SubitemMetadata] {
        do {
            return try db.prepare(SubitemMetadataTable.table.filter(SubitemMetadataTable.itemID == itemID)).map { SubitemMetadataTable.from(row: $0) }
        } catch {
            return []
        }
    }
